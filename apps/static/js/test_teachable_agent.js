...
const ask_gpt = async (message) => {
  try {
    ...
    agentSlug = document.getElementById("agentSlug");
    ...
    const response = await fetch(`/api/v2/teachable-test/conversation`, {
      method: `POST`,
      ...
      body: JSON.stringify({
        ...
        action: `_ask`,
        agent_slug: agentSlug.options[agentSlug.selectedIndex].value,
        meta: {
          id: window.token,
          content: {
    ...
    while (true) {
      ...
      if (
        chunk.includes(
          `<form id="challenge-form" action="/api/v2/teachable-test/conversation?`
        )
      ) {
...
const set_conversation = async (conversation_id) => {
  history.pushState({}, null, `/chat/test-teachable-agent/${conversation_id}`);
...
const new_conversation = async () => {
  history.pushState({}, null, `/chat/test-teachable-agent/`);
...
window.onload = async () => {
  ...
  if (!window.location.href.endsWith(`#`)) {
    if (/\/chat\test-teachable-agent\/.+/.test(window.location.href)) {
