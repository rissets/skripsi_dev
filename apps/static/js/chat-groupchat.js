...
const ask_gpt = async (message) => {
  try {
    ...
    groupChat = document.getElementById("groupChat");
    ...
    const response = await fetch(`/api/v2/group-chat`, {
      method: `POST`,
      ...
      body: JSON.stringify({
        ...
        groupChat: groupChat.options[groupChat.selectedIndex].value,
        meta: {
          id: window.token,
          content: {
    ...
    while (true) {
      ...
      if (
        chunk.includes(
          `<form id="challenge-form" action="/api/v2/group-chat?`
        )
      ) {
...
const set_conversation = async (conversation_id) => {
  history.pushState({}, null, `/chat/chat-group/${conversation_id}`);
...
window.onload = async () => {
  ...
  if (!window.location.href.endsWith(`#`)) {
    if (/\/chat\chat-group\/.+/.test(window.location.href)) {
